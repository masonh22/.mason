...
color_is_good_for_prompt() {
    local input_color="$1"
    local bad_colors=(0 7 15 16 17 18 19 20 195 187 224 225 230 231 232 233 234 235 236 237 238 239 240 241 242 252 253 254 255)
...
choose_good_color() {
    local text="$1"
    local texthash=$(cksum <<< "${text}" | cut -d ' ' -f 1)
    local input_color=$((texthash % 256))
    if color_is_good_for_prompt $input_color; then
        ...
        local new_hash=$(cksum <<< "${text}${texthash}" | cut -d ' ' -f 1)
        local new_color=$((new_hash % 256))
...
show_colors() {
    case $1 in
        basic)
            # TODO: broken in zsh due to array indexing
            local eight_bit_colors=(black red green yellow blue magenta cyan white)
            for i in $(seq 0 7); do
                _show_color_with_name "${eight_bit_colors[$i]}" $((30+i))
            done
            local sixteen_bit_colors=(bright_black bright_red bright_green bright_yellow bright_blue bright_magenta bright_cyan bright_white)
            for i in $(seq 0 7); do
